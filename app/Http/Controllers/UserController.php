...
//            'nickname' =>  'required|between:2,12',
    ...
    public function checkUser( Request $request, $identifier )
    {
        ...
        if( $userIdentifier = UserIdentifiers::findByIdentifier( $identifier ) ) {
            ...
            $user->is_register = $user->uc_id ? true: false;
            $user->identifier = $userIdentifier->identifier;
            $ucUser = [];
            if($user->uc_id){
                 $ucUser = UcUser::findById($user->uc_id);
            }
            $user             = $user->toArray();
            $user['is_mute']  = self::isMute($user['id']);
            $user['id']       = hashid( $user['id'], 'user' );
            ...
            return response()->json( ['status' => 404, 'messge' => '查询不到用户信息'] );
        }
    }
    ...
    public function bindPhone( Request $request )
    {
        ...
        //写入
        $state = UcUser::where('id',$info['uc_id'])->update(['phone'=>$request->input('mobile')]);
        if($state){
    ...
    public function unbindPhone( Request $request )
    {
        ...
        //解绑
        $state = UcUser::where('id',$info['uc_id'])->update(['phone'=>'']);
        if($state){
            Redis::del($info['cacheKey']);
    ...
    private static function verifyBind($request, $user,$type)
    {
        ...
        if( !$ucUser = UcUser::findById($user->uc_id) ) {
            throw new UserException( 404,'用户异常，请重新登录' );
        }
        ...
        if($type ==1 && $ucUser->phone !=''){
            throw new UserException( 406,'已绑定手机，请先解绑' );
        }

        $isset = UcUser::findByPhone($phone);
        //type为1  绑定  判断手机是否已绑定
        if($type == 1 && $isset){
            throw new UserException( 406,'手机已绑定' );
        }

        //type为2 解绑  判断手机是否存在
        if($type == 2 && !$isset){
            throw new UserException( 407,'手机号异常，请重新登录' );
        }
        ...
        return ['cacheKey'=>$cacheKey,'uc_id'=>$ucUser->id];
    }
    ...
    public function resetPassword( Request $request )
    {
        ...
        $state = UcUser::where('phone',$phone)->update($data);
    ...
    public function getVerifyCode( Request $request, $type, $phone)
    {
        ...
        if( $type == 1 && $ucUser = UcUser::findById($user->uc_id) ){
            if($ucUser->phone !=''){
                return response()->json(['status'=>405,'message'=>'用户已绑定手机，异常状态，请重新登录']);
            }
        }
        //type为2 获取解绑验证码  判断手机是否存在
        if( $type == 2 && !$isset ){
            return response()->json(['status'=>407,'message'=>'手机号异常，请重新登录']);
        }
        //type为3 检测手机号是否已绑定
        if( $type == 3 && !$isset ){
            return response()->json(['status'=>407,'message'=>'修改密码失败，您的手机没有绑定任何账号']);
        }
