...
//        $payList = Pay::getPayMethod($os);
        ...
        $PayList = Pay::getPayListByApi("2");
        $payList = $PayList['payList'];
        $payChannel = json_encode($PayList['payChannel'],320);
        $orderUrl = "http://".env("APP_GETPAYLIST")."/api/pay/order?";
        $apiToken = $request->header('api-token');
//        $apiToken = '03b9518e6996f3d5daa4821b25fd9662';
        $vips = Vip::getList();
        $configs = Config::getConfig();
        $payBanner = $configs['pay_banner_img']['content'] ? $configs['pay_banner_img']['content'][0] : 0 ;
        ...
        if( $user = \App\Model\User::findByApiToken($apiToken) ){
            $uc_id = $user->uc_id;
            $uid = $user->id;
        } else {
            $uc_id = 0;
        ...
        //检测半自动扫码是否开启
        try{
            $apiUrl = "http://".env("APP_GETPAYLIST")."/api/pay/qrStatus?project_id=2";
		...
		return view('pay.pay',compact('vips','payBanner','payNotice','payList', 'payChannel', 'os','apiToken','customerService','channel','orderUrl','uid','uc_id', 'wechatQr', 'alipayQr', 'qqQr', 'jdQr', 'bankQr'));
	}
    ...
    public function notifySuccess( Request $request ){
        ...
        if($uid == "" || $money == "" || $orderId == ""){
            return response()->json( ['status'=>20001,'message'=>'参数错误'] );
        }

        //获取下单时的item_info
        $type = 'vip';
        $itemInfo = Order::getItemInfo($type, $itemId);

        // users_vips表插入记录
        $result = UserVip::createVip($uid, $itemInfo, $orderId, $money);

        //记录推广红利
        $user = Order::getUserInfo($uid);
